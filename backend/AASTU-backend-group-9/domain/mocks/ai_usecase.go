...
func (_m *AIUsecase) GenerateBlogContent(ctx context.Context, keywords string) ([]genai.Part, error) {
	ret := _m.Called(ctx, keywords)

	var r0 []genai.Part
	if rf, ok := ret.Get(0).(func(context.Context, string) []genai.Part); ok {
		r0 = rf(ctx, keywords)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]genai.Part)
		}
	}
