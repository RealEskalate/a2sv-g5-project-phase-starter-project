...
// SendOTP provides a mock function with given fields: c, user, username, password, devicePrint
func (_m *SignupUsecase) SendOTP(c context.Context, user *domain.AuthSignup, username string, password string, devicePrint string) error {
	ret := _m.Called(c, user, username, password, devicePrint)

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, *domain.AuthSignup, string, string, string) error); ok {
		r0 = rf(c, user, username, password, devicePrint)
	} else {
		r0 = ret.Error(0)
	}
