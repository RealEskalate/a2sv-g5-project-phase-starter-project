...
func (suite *AuthControllerTestSuite) TestLogout() {
	suite.router.POST("/logout", func(c *gin.Context) {
		// Setting the userId in the Gin context
		c.Set("userId", "userID")
		suite.authController.Logout(c)
	})

	// Mock the Logout method to return nil
	...
	// Check if the Logout method was called with the correct arguments
...
func (suite *AuthControllerTestSuite) TestVerifyEmail() {
	...
	// Check that the VerifyEmail method was called with the correct arguments
	suite.authServiceMock.AssertCalled(suite.T(), "VerifyEmail", "test@example.com", "123456")
	suite.authServiceMock.AssertExpectations(suite.T())
}
