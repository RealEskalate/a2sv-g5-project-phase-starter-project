...
func (br *blogUsecase) UpdateFeedback(ctx context.Context, id string, updateFunc func(*domain.Feedback) error) error {
	feedback, err := br.blogRepository.FetchFeedbackByID(ctx, id)
	if err != nil {
		return err
	}
...
func (blogU *blogUsecase) UpdateComment(ctx context.Context, blogID primitive.ObjectID, userID string, updatedComment domain.Comment) error {
...
func (blogU *blogUsecase) RemoveComment(c context.Context, blogID primitive.ObjectID, requesterUserID string) error {
	...
	ctx, cancel := context.WithTimeout(ctx, br.contextTimeouts)
	defer cancel()

	return br.blogRepository.UpdateFeedback(ctx, id, feedback)
}
