...
var (
	...
	ErrInvalidSearchingRequest = errors.New("invalid searching request")
)
...
// validation for BlogCreate
func validateContetnt(content string) bool {
	if len(content) < 10 {
		return false
	}
	return true
...
func (b *BlogCreate) Validate() error {
	if !validateContetnt(b.Content) {
		return ErrInvalidContent
	}
	if !validateTitle(b.Title) {
		return ErrInvalidTitle
	}
	if !validateTags(b.Tags) {
		return ErrInvalidTags
	}
	return nil
}

func (b *BlogUpdate) Validate() error {
	if !validateContetnt(b.Content) {
		return ErrInvalidContent
	}
	if !validateTitle(b.Title) {
...
// validation for BlogSearchRequest
func validateSearchRequest(searchRequest *BlogSearchRequest) bool {
...
func (b *BlogSearchRequest) Validate() error {
	if !validateSearchRequest(&BlogSearchRequest{Title: b.Title, Author: b.Author}) {
		return ErrInvalidSearchingRequest
	}
	return nil
}
