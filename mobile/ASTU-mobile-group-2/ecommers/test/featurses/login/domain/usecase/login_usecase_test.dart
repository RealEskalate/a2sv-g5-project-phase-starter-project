

import 'dart:convert';

import 'package:dartz/dartz.dart';
import 'package:ecommers/features/login/data/model/model.dart';
import 'package:ecommers/features/login/domain/entity/login_entity.dart';
import 'package:ecommers/features/login/domain/usecase/login_usecase.dart';
import 'package:flutter_test/flutter_test.dart';
import 'package:mockito/mockito.dart';

import '../../../../helper/dummy_data/read_json.dart';
import '../../../../helper/test_hlper.mocks.dart';


void main() {
  late MockLoginRepositories mockLoginRepository;

  late LoginUseCase loginUseCase;


  setUp (() {
    mockLoginRepository = MockLoginRepositories();
    loginUseCase = LoginUseCase(repository: mockLoginRepository);
  });



  final data = readJson('helper/dummy_data/login_success.json');
  final jsonDecode = json.decode(data);
  final LoginModel loginModel = LoginModel.fromJson(jsonDecode);
  final LoginEntity loginEntity = loginModel.toEntity();


  group(
    'test the usecase',
    () {
      test(
        'the usecase must return success login',
        () async {
            when(mockLoginRepository.login('samuel.tolossa@a2sv.org', '123qweasdzxc')).thenAnswer((_) async => Right(loginEntity));
            final result = await loginUseCase.loginUser('samuel.tolossa@a2sv.org', '123qweasdzxc');
            

            // Check the result type
         expect(result, isA<Right>());
         expect(result.fold((l) => l, (r) => r), isA<LoginEntity>());
          
        }
      );
    }
    );

}