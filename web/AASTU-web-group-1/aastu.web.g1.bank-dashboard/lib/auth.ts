...
export async function signInWithCredentials(credentials: Credentials): Promise<SignInResponse> {
  return await ky.post("https://bank-dashboard-o9tl.onrender.com/auth/login", {
    json: credentials,
  }).json<SignInResponse>();
}
...
export async function refreshAccessToken(refreshToken: string): Promise<string | null> {
    try {
      const response = await ky.post('https://bank-dashboard-o9tl.onrender.com/refresh-token', {
        json: { refreshToken },
