...
import { IoSettingsOutline, IoSearchOutline, IoNotificationsOutline } from "react-icons/io5";
...
const SidebarWithNavbar = () => {
  ...
  return (
    <>
      <aside
        className={`fixed top-0 left-0 z-40 min-w-60 w-64 h-full bg-white shadow-md transition-transform transform ${
            ...
            <Link href={item.href} key={item.title}>
              ...
              >
                <Image
                  ...
                  height={24}
                  className={`w-6 h-6 ${
                    activeItem === item.title ? "filter-active" : "filter-inactive"
                  }`}
                />
                <div>{item.title}</div>
              </li>
