...
const RecentTransactions = () => {
  ...
  const handleNextPage = () => {
    if (
      ...
      allPage < allData!.data.totalPages
    ) {
      setAllPage(allPage + 1);
    } else if (
      activeTab === "income" &&
      incomePage < incomeData!.data.totalPages
    ) {
      setIncomePage(incomePage + 1);
    } else if (
      activeTab === "expenses" &&
      expensePage < expenseData!.data.totalPages
    ) {
      setExpensePage(expensePage + 1);
    }
  };
  ...
  const determineAmountSign = (transaction: Transaction) => {
    if (transaction.type === "service" || transaction.type === "shopping") {
      return -Math.abs(transaction.amount);
    } else if (
      transaction.type === "deposit" ||
      transaction.senderUserName !== "current_user"
    ) {
      return Math.abs(transaction.amount);
    } else if (transaction.senderUserName === "current_user") {
      return -Math.abs(transaction.amount);
    } else {
  ...
  const renderContent = () => {
    ...
    return transactions.map((transaction, index) => ({
      ...transaction,
      amount: determineAmountSign(transaction),
    }));
  };
  ...
  return (
    <div className="mt-4 mx-auto">
      <h1 className="text-xl font-semibold mb-4 text-[#343C6A] dark:text-white ">
        Recent Transactions
      </h1>
      ...
      <section className="border-0 rounded-xl bg-white dark:bg-darkComponent shadow-md p-2">
        ...
        {renderContent().map((transaction, index) => (
          ...
          >
            ...
            <div className="hidden lg:block sm:text-xs">
