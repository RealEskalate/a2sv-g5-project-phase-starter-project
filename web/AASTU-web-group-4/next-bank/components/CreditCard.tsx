    ...
    <div className={`${backgroundColor} w-[231px] h-[170px] sm:w-[265px] sm:h-[170px] md:w-[350px] md:h-[235px] rounded-xl relative ${backgroundColor === colors.white ? 'border-[1px] border-gray-300' : ''}`}>
      <div className="flex justify-between w-[95%]">
        <div className='mt-1 ml-3 p-2'>
            ...
            <span className={`block text-[16px] sm:text-[16px] md:text-[20px] font-bold ${backgroundColor == colors.blue ? 'text-white' : 'text-black'}`}>{balance}</span>
        </div>
      ...
      <div className="flex justify-between w-[90%]">
        <div className='ml-3 pl-1.5 md:p-2'>
            ...
            <span className={`block text-[13px] md:text-[15px] font-bold ${backgroundColor == colors.blue ? 'text-white' : 'text-black'}`}>{cardHolder}</span>
        </div>
        ...
        <div className='mr-3 md:mr-9 md:p-2'>
            ...
            <span className={`block text-[13px] md:text-[15px] font-bold ${backgroundColor == colors.blue ? 'text-white' : 'text-black'}`}>{expiryDate}</span>
        </div>
      </div>
      ...
      <div className={`flex justify-between child-div absolute bottom-0 left-0 right-0 ${backgroundColor == colors.blue ? 'bg-gradient-to-b from-blue-600' : 'border-t-[1px]'}`}>
        <span className={`p-3 ml-2 text-[15px] sm:text-[15px] md:text-[22px] ${backgroundColor == colors.blue ? 'text-white' : 'text-black'}`}>{cardNumber}</span>
