  ...
  return (
    <div>
     ...
     <div className="hidden md:block overflow-x-auto">
        <table className="min-w-full bg-white rounded-full shadow-lg border-gray-200">
          <thead className="bg-blue-50  dark:bg-dark">
            <tr className="dark:text-blue-500 text-gray-700">
              <th className="p-4 text-left text-sm font-semibold">Description</th>
              <th className="p-4 text-left text-sm font-semibold">Transaction ID</th>
              <th className="p-4 text-left text-sm font-semibold">Type</th>
              <th className="p-4 text-left text-sm font-semibold">Card</th>
          ...
          <tbody>
            {alltransaction.map((transaction : any, index:any) => (
              <tr
                ...
                className={`border-b border-gray-200 ${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} hover:bg-gray-100 transition-colors duration-300  dark:bg-dark text-gray-900 dark:text-white`}
              >
                ...
                <td className="p-4 text-sm text-gray- dark:text-white">{transaction.transactionId}</td>
                <td className="p-4 text-sm text-gray- dark:text-white">{transaction.type}</td>
                <td className="p-4 text-sm text-gray- dark:text-white">Card Name</td>
                <td className="p-4 text-sm text-gray- dark:text-white">{transaction.date}</td>
      ...
      <div className="block md:hidden">
        {alltransaction.map((transaction:any) => (
          <div key={transaction.transactionId} className="flex justify-between bg-white p-4 mb-2 rounded-lg shadow-sm border border-gray-200 items-center  dark:bg-dark text-gray-900 dark:text-white">
            <div>
