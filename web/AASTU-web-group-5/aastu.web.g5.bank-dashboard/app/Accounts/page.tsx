...
const Accounts = () => {
  ...
  const fetchIncomes = async (accessToken: string) => {
    try {
      ...
      if (response.data.success) {
        setIncome(response.data.data.content.reduce((total: number, income: any) => total + income.amount, 0));
      }
    } catch (error) {
  ...
  const fetchExpenses = async (accessToken: string) => {
    try {
      ...
      if (response.data.success) {
        setExpense(response.data.data.content.reduce((total: number, expense: any) => total + expense.amount, 0));
      }
    } catch (error) {
  ...
  return (
    <div className="bg-[#F5F7FA] dark:bg-gray-900 space-y-8 w-[95%] pt-3 overflow-hidden mx-auto">
      ...
      {/* Last Transactions */}
      <div className={`p-4 rounded-lg lg:col-span-2 space-y-7 ${darkmode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
      ...
      {/* Card */}
      <div className={`rounded-lg ${darkmode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`rounded-lg lg:col-span-2 ${darkmode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
          <p className="text-lg font-semibold">Debit and credit overview</p>
          {loading ? <Shimmer1 /> :
            (<>
              <div className={`p-1 mt-3 rounded-lg shadow ${darkmode ? 'bg-gray-700' : 'bg-white'}`}>
                <ChartCard />
              </div>
            </>)
          }
        ...
        {/* Invoices Sent */}
        <div className={`w-full rounded-lg ${darkmode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
          <p className="text-lg font-semibold">Invoices sent</p>
          {loading ? <Shimmer1 /> :
            (<>
              <div className={`p-4 mt-4 rounded-lg shadow h-[364px] ${darkmode ? 'bg-gray-700' : 'bg-white'}`}>
