...
useEffect(() => {
  const fetchInvestmentData = async () => {
    ...
    try {
      ...
      const { totalInvestment, rateOfReturn, yearlyTotalInvestment, monthlyRevenue } = response.data.data;
    ...
    <div className="bg-[#F5F7FA] dark:bg-gray-900  space-y-8 w-[95%] pt-3 overflow-hidden mx-auto">
      {/* Row 1: Investment Overview */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            {loading ? <Shimmer1 /> : (
              <div className={`p-4 rounded-lg flex items-center justify-center space-x-4 ${darkmode ? 'bg-gray-800 text-white' : 'bg-white text-black'}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className={`p-4 rounded-lg ${darkmode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
            <p>Yearly Total Investment</p>
              ...
              {loading ? <Shimmer1 /> : (
                <div className={`h-36 rounded mt-4 ${darkmode ? 'bg-gray-700' : 'bg-white'}`} style={{ width: "100%", height: 329 }}>
      ...
      {/* Row 3: Investments and Trading Stock */}
        <div className="flex flex-col md:flex-row gap-4">
          {/* Investments Section */}
            <div className={`md:w-[58%] p-4 rounded-lg min-h-[345px] ${darkmode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
              <p className="text-lg font-semibold">My Investments</p>
              <div className="space-y-4 mt-4">
                {investmentsData.slice(0, 3).map((investment) => (
                  <div
                    key={investment.id}
                    className={`flex items-center space-x-4 p-2 rounded-lg shadow ${darkmode ? 'bg-gray-700' : 'bg-white'}`}
                  >
                    {loading ? (
                      <Shimmer1 />
                    ) : (
                      <>
                        ...
                        <div className="flex-1">
                          ...
                          <p className={`text-gray-500 ${darkmode ? 'text-gray-400' : ''}`}>{investment.service}</p>
                        </div>
                        <div>
                          <p className="text-sm font-semibold">{investment.value}</p>
                          <p className={`text-xs ${darkmode ? 'text-gray-400' : 'text-gray-500'}`}>Investment value</p>
                        </div>
                        <div>
                          <div>
                            {investment.return < 0 ? (
                          ...
                          <p className={`text-xs ${darkmode ? 'text-gray-400' : 'text-gray-500'}`}>Return</p>
                        </div>
                      </>
                    )}
                  </div>
          ...
          <div className={`md:w-[42%] p-4 rounded-lg min-h-[345px] ${darkmode ? 'bg-gray-800 text-white' : 'bg-gray-100 text-black'}`}>
            <p className="text-lg font-semibold">Trading Stock</p>
            <div className="mt-4">
              <table className={`w-full rounded-lg shadow ${darkmode ? 'bg-gray-700' : 'bg-white'}`}>
                <thead>
