...
const NavBar = ({ toggleSidebar, isSidebarVisible }) => {
  ...
  const [profilePicture, setProfilePicture] = useState(
    reduxUser?.profilePicture && reduxUser?.profilePicture.startsWith('https') ? reduxUser?.profilePicture : "/images/christina.png"
  );
  ...
  useEffect(() => {
    if (reduxUser?.profilePicture && reduxUser?.profilePicture.startsWith('https')) {
  ...
  return (
    <div className={`shadow-md bg-white dark:bg-gray-900`}>
      ...
      <div className="hidden sm:flex justify-between items-center h-24 px-12">
        <div className={`font-semibold text-xl ${darkmode ? "text-gray-200" : "text-primary-2"}`}>
        ...
        <div className="flex gap-5 items-center">
          <form onSubmit={handleSearchSubmit} className="relative w-full max-w-xs">
            <input
              ...
              value={searchText}
              onChange={(e) => setSearchText(e.target.value)}
              className={`pl-12 pr-12 py-2 w-full ${darkmode ? "bg-gray-700 text-white" : "bg-gray-100 text-black"} rounded-full border border-gray-300`}
            />
            <div className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-500">
              <Image src={magnifying} alt="Search Icon" width={16} height={16} />
            </div>
          ...
          <button onClick={handleToggleDarkMode}>
            <Image src={darkmode ? lightModeImg : darkModeImg} alt="Toggle Dark Mode" width={45} height={45} />
          </button>
          ...
          />
          <button onClick={() => signOut()} className="bg-slate-500 text-white rounded-full p-4 py-2 ml-4">
            Logout
          </button>
        </div>
      </div>
