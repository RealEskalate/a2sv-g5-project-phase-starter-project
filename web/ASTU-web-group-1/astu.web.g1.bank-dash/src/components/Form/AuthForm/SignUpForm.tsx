...
const stepOneSchema = z
  .object({
    ...
    username: z.string({ required_error: 'Username is required' }).min(4, 'Username is too short'),
...
const SignUpForm = () => {
  ...
  return (
    ...
    >
      <p className='text-[#333B69] pb-3 text-28px text-left font-semibold w-full'>Register</p>
      {step == 0 && (
        <>
      ...
      <div className={`w-full flex ${step == 0 ? 'justify-end' : 'justify-between'}`}>
        {step > 0 && (
          <button
            type='submit'
