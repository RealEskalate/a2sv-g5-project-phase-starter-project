'use client';
import { CardContentType, CardResponseType } from '@/types/card.types';
import Image from 'next/image';
import React from 'react';


const MyCard = ({ content, index }: { content: CardContentType; index: number }) => {
          ...
          <div className='flex h-full pb-3'>
            ...
            <div className='flex flex-1 flex-col justify-around items-center'>
              <div className='flex flex-col'>
                ...
                <span className='text-14px  font-semibold'>{formatTime(expiryDate)}</span>
              </div>
            </div>
          </div>
        </div>
        <div className={`${gradient} ${border} rounded-b-3xl flex justify-between px-6 py-3 `}>
          <div className='flex items-center'>
            <span className='text-16px font-text-navy'>{formatCardNumber(cardNumber)}</span>
          </div>
